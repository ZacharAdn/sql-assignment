...
create TEMPORARY TABLE if not exists table1 AS(
    ...
    on sol.cityid=fam.cityid and cit.cityid=sol.cityid and fam.id=famLan.id and sol.personalid=solLan.personalid and solLan.lng=famLan.lng);
...
create TEMPORARY TABLE if not exists table3 AS ( 
	select  distinct fam3.familyname as fittedfamilies ,cit3.cityname,cit3.RegID, famLan.lng,
     fam3.phone1 as phone1, fam3.phone2 as phone2, fam3.id as famID
	from families as fam3 ,city as cit3,LanguageFamilies as famLan
	where fam3.cityid=cit3.cityid and famLan.id=fam3.id and fam3.id not in(select id from table1));
...
create TEMPORARY TABLE if not exists table4 AS(
	...
	t3.phone1, t3.phone2, t2.solider_phone , t2.personalid , t3.famID as famID
	from  table2 as t2 join table3 as t3
	where t2.regid=t3.regidand t2.lng=t3.lng);

select * from families where id not in (select id from table1 union all select famID from table4);
